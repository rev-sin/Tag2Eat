  ...
  const columns = isSignedIn
    ? [
        ...baseColumns,
        {
          ...
          header: "Actions",
          cell: ({ row }: any) => {
            const inCart = items.some((i) => i.id === row.original.id);
            ...
            return (
              <div className="flex justify-center">
                {inCart ? (
                  <button
                    disabled
                    className="bg-gray-400 text-white px-3 py-1 rounded opacity-60 cursor-not-allowed"
                  >
                    Added
                  </button>
                ) : (
          ...
          {table.getHeaderGroups().map((headerGroup) => (
            <tr key={headerGroup.id}>
              {headerGroup.headers.map((header) => (
                <th
                  key={header.id}
                  className="border-b border-gray-300 px-4 py-2 text-center text-sm font-semibold"
                >
                  {flexRender(
          ...
          {table.getRowModel().rows.map((row) => (
            <tr key={row.id} className="hover:bg-gray-50">
              {row.getVisibleCells().map((cell) => (
                <td
                  key={cell.id}
                  className="border-b border-gray-200 px-4 py-2 text-center text-sm"
                >
